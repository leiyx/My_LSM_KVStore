...
int32_t FreeListAlloc::M_FreelistIndex(int32_t bytes) {
  // first fit策略
  return (bytes + kAlignBytes - 1) / kAlignBytes - 1;
}
int32_t FreeListAlloc::M_Roundup(int32_t bytes) {
  // 向上取整
  return (bytes + kAlignBytes - 1) & ~(kAlignBytes - 1);
}
char* FreeListAlloc::M_ChunkAlloc(int32_t bytes, int32_t& nobjs) {
  char* result = nullptr;
  // 总的字节大小
...
void* FreeListAlloc::M_Refill(int32_t bytes) {
...
void* FreeListAlloc::Allocate(int32_t n) {
  assert(n > 0);
...
void FreeListAlloc::Deallocate(void* address, int32_t n) {
  if (address) {
...
void* FreeListAlloc::Reallocate(void* address, int32_t old_size,
                                int32_t new_size) {
